...
- The `min` matters whenever `num < prev + 1` — duplicates (`num == prev`), drops (`num < prev`), and ties (`num == prev + 1`) all keep `num`; only `num > prev + 1` triggers the decrease down to `prev + 1`.
